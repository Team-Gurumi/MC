...
MC_DB_DSN="${MC_DB_DSN:-}" # <--- 수정됨: MC_DB_DSN을 스크립트 변수로 읽어오기
...
[[ -n "${MC_DB_DSN}" ]] && CONTROL_ENV+=(MC_DB_DSN="${MC_DB_DSN}") # <--- 수정됨: MC_DB_DSN을 컨트롤 환경에 전달
...
for i in {1..50}; do
  ...
  sleep 0.1
  # <--- 수정됨: 컨트롤 서버가 DB 연결 실패 등으로 바로 죽었는지 확인
...
body="{\"id\":\"${JOB}\",\"image\":\"${DOCKER_IMAGE}\",\"command\":[\"/bin/sh\",\"-lc\",\"echo agent:$RANDOM && sleep 10 && echo done\"]}"
  ...
  if [[ -n "${PROVIDERS_JSON}" ]]; then
    ...
    # <--- 수정됨: 사용자의 수동 명령어와 동일한 동적 Manifest 구조 사용
    mbody=$(cat <<EOF
{
  "root_cid": "noop",
  "providers": [],
